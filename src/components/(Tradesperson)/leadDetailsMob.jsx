...
const LeadDetailsMob = ({ leadData, openModal, updateLeadCount }) => {
  ...
  const getUser = async (id) => {
    const userjwt = await getUserDetails();
    if (!id) return;
    try {
      setLoading(true)
  ...
  return (
    ...
    <div style={{ height: 'auto' }} className="overflow-y-scroll p-2 w-full bg-transparent">
      {isLoading ? (
          ...
          <div className="flex flex-col space-y-2 sm:space-y-0 sm:flex-row sm:space-x-4 md:flex-col md:space-x-0 md:space-y-2 items-start md:items-end">
            ...
            {reveal ? '' : <>
              {revealLoading ?
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); checkReveal() }}
                  className="w-full sm:w-auto bg-yellow-500 text-black text-sm sm:text-base font-semibold px-4 sm:px-6 py-2 rounded-md border border-gray-300"
                >
                  Reveal
                </button>
